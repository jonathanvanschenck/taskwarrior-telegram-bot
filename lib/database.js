...
class DB {
    constructor(config, log) {
        ...
        if ( this.config.data ) {
            this.log.info(`Using data directory: ${this.config.data}`);
            try {
                mkdirSync(this.config.data, { recursive: true });
            } catch (err) {
                this.log.error(`Failed to create data directory: ${err.message}`);
                throw err;
            }
        } else {
            this.log.warn('No data directory specified, using in-memory database (data will not persist across restarts)');
        }
        this.db = new sqlite(
            this.config.data ? join(this.config.data, 'db.sqlite') : ':memory:',
            { fileMustExist: false }
        );
    }
