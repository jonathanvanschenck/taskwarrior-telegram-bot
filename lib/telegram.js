...
class Telegram {
    constructor(config, log, taskwarrior, db) {
        this.config = config;
        this.log = log;
        this.tw = taskwarrior;
        this.db = db;
    ...
    push_notification(message, opts={}) {
        const chats = this.db.get_chats()
        const unique_chats = [...new Set(chats.map(c => c.chat_id))];
        this.log.info(`Pushing notification to ${unique_chats.length} chat(s)`);
        unique_chats.forEach((chat_id) => {
            this.bot.telegram.sendMessage(chat_id, message, opts).catch(err => {
                this.log.error(`Failed to send message to chat ${chat_id}: ${err.message}`);
            });
        });
    }
    ...
    async launch() {
        ...
        this.set_command('start', '/start - configure this bot to talk with you on this chat', this._handle_start);
        this.set_command('stop', '/stop - configure this bot to stop talking with you on this chat', this._handle_stop);
        ...
        this.set_command('end', '/end <id|uuid> - stop a task', this._handle_stop_task);
    ...
    _handle_start(ctx) {
        ...
        if ( user_id && chat_id ) {
            this.log.info(`Registering chat ${chat_id} for user ${user_id}`);
            this.db.add_chat(user_id, chat_id);
        }
    ...
    _handle_stop(ctx) {
        ...
        const chat_id = ctx.chat ? ctx.chat.id : null;
        if ( user_id && chat_id ) {
            this.log.info(`Unregistering chat ${chat_id} for user ${user_id}`);
            this.db.remove_chat(user_id, chat_id);
        }
        this._wrap_reply(ctx, 'Goodbye! I will stop sending you updates. You can start again anytime by sending /start');
    }
    ...
    async _handle_sync(ctx) {
        const resp = await this.tw.sync();
        this._wrap_reply(ctx, resp);
    }

    async _handle_undo(ctx) {
        const resp = await this.tw.undo();
        this._wrap_reply(ctx, resp);
    }
