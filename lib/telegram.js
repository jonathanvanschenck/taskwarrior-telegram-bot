...
class Telegram {
    ...
    set_command(cmd, doc, handler) {
        ...
        switch (cmd) {
            case 'start':
                this.bot.start(this._wrap_handler(handler));
                break;
            case 'help':
                this.bot.help(this._wrap_handler(handler));
                break;
            default:
                this.bot.command(cmd, this._wrap_handler(handler));
                break;
        }
    }
    ...
    async launch() {
        ...
        this.set_command('start', undefined, this._wrap_handler(this._handle_start));
        this.set_command('help', '/help - show this menu', this._wrap_handler(this._handle_help));
        this.set_command('version', '/version - get the version', this._wrap_handler(this._handle_version));
        this.set_command('list', '/list [filter] - list tasks matching a filter', this._wrap_handler(this._handle_list));
        this.set_command('info', '/info <id|uuid> - show info on a task', this._wrap_handler(this._handle_info));
        this.set_command('add', '/add <description> [mods] - add a new task', this._wrap_handler(this._handle_add));
        this.set_command('begin', '/begin <id|uuid> - start a task', this._wrap_handler(this._handle_start_task));
        this.set_command('stop', '/stop <id|uuid> - stop a task', this._wrap_handler(this._handle_stop_task));
        this.set_command('sync', '/sync - run synchronization with the server', this._wrap_handler(this._handle_sync));
        this.set_command('undo', '/undo - undo the last operation', this._wrap_handler(this._handle_undo));
        this.set_command('modify', '/modify <id|uuid> <mods> - modify a task', this._wrap_handler(this._handle_modify));
        this.set_command('delete', '/delete <id|uuid> - delete a task', this._wrap_handler(this._handle_delete));
        this.set_command('done', '/done <id|uuid> - mark a task complete', this._wrap_handler(this._handle_done));
        this.set_command('annotate', '/annotate <id|uuid> <annotation - add an annotation to a task', this._wrap_handler(this._handle_annotate));
    ...
    async _handle_start_task(ctx) {
        const msg = extract_command_args('start', ctx.message?.text);
        const [id] = extract_id(msg);
        const resp = await this.tw.start(id);
        this._wrap_reply(ctx, resp);
    }
    async _handle_stop_task(ctx) {
        const msg = extract_command_args('stop', ctx.message?.text);
        const [id] = extract_id(msg);
        const resp = await this.tw.stop(id);
        this._wrap_reply(ctx, resp);
    }
    async _handle_modify(ctx) {
        const msg = extract_command_args('modify', ctx.message?.text);
    ...
    async _handle_delete(ctx) {
        const msg = extract_command_args('delete', ctx.message?.text);
        const [id] = extract_id(msg);
        const resp = await this.tw.delete(id);
        this._wrap_reply(ctx, resp);
    }
    async _handle_done(ctx) {
        const msg = extract_command_args('done', ctx.message?.text);
        const [id] = extract_id(msg);
        const resp = await this.tw.done(id);
        this._wrap_reply(ctx, resp);
    }
    async _handle_annotate(ctx) {
        const msg = extract_command_args('annotate', ctx.message?.text);
        const [id, rest] = extract_id(msg);
