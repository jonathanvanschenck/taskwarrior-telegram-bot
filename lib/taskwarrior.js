...
class Taskwarrior {
        ...
        return new Promise((resolve, reject) => {
            ...
            process.on('close', (code) => {
                if (code === 0) {
                    ...
                    reject(new ExecutionError(code, stderr));
                }
            });
    ...
    async undo() {
        return this._execute(['undo']).then(stdout => {
            return stdout.trim();
        });
    }
    ...
    async list(filter, subcommand) {
        return this._execute([...(filter ? [filter] : []), ...(subcommand?[subcommand]:[])]).then(stdout => {
            return stdout.trim();
        }).catch(err => {
            if (err instanceof ExecutionError && err.code === 1) {
    ...
    async start(id) {
        const parsed_id = parse_id(id);
        return this._execute([parsed_id, 'start']).then(stdout => {
            return stdout.trim();
        });
    }
    ...
    async stop(id) {
        const parsed_id = parse_id(id);
        return this._execute([parsed_id, 'stop']).then(() => {
            return this._execute([parsed_id, 'done']);
        });
    }

    async done(id) {
        const parsed_id = parse_id(id);
        return this._execute([parsed_id, 'done']).then(() => {
            return this._execute([parsed_id, 'done']);
        });
    }
    ...
    async delete(id) {
        const parsed_id = parse_id(id);
        return this._execute([parsed_id, 'done']).then(() => {
            return this._execute([parsed_id, 'delete']);
        });
    }

    async annotate(id, str) {
        const parsed_id = parse_id(id);
        return this._execute([parsed_id, 'annotate', str]).then(stdout => {
            return stdout.trim();
        });
    }

    async calc(expr) {
        return this._execute(['calc', expr]).then(stdout => {
            return stdout.trim();
        });
    }
    ...
    async add(str, mods=[]) {
        ...
        return this._execute(['add', description, ..._mods]).then(stdout => {
            return stdout.trim();
        });
    }
